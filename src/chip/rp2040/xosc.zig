...
//!
//! Source: [RP2040 datasheet §2.16, "Crystal Oscillator (XOSC)"](https://datasheets.raspberrypi.com/rp2040/rp2040-datasheet.pdf).
...
/// was written; only the named value commands a range on this chip.
pub const FreqRange = enum(u12) {
    /// 1-15 MHz. The only band the RP2040 XOSC implements.
    mhz_1_15 = 0xaa0,
    _,
};
...
/// `COUNT`: a down-counter clocked by the crystal, for software timing loops
/// before any other clock is trustworthy. Eight bits wide on this chip.
...
/// XOSC register block.
///
/// `COUNT` sits at `0x1c`, three reserved words past `STARTUP`, and contains an
/// eight-bit counter.
...
/// The XOSC peripheral at its RP2040 base address.
...
comptime {
    ...
    std.debug.assert(@bitSizeOf(@FieldType(Count, "count")) == 8);
    ...
    std.debug.assert(@offsetOf(Registers, "count") == 0x1c);
